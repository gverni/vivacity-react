  ...
  function statementToWeeklyPoints (strStatement) {
    ...
    if (statementItems.length) {
      statementItems.forEach((item) => {
        let fullDate = item.date + ' ' + ((new Date(item.date + (new Date()).getFullYear()) > new Date()) ? (new Date()).getFullYear() - 1 : (new Date()).getFullYear())
        let dateWeekNo = (new Date(fullDate)).getWeek()
